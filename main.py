...
def get_optimal_atr_mult(df):
    ...
    # [퀀트 방패] 과거 기회가 10번 미만이면 데이터 부족으로 판단, 임의 숫자 부여 금지
    if len(mae_list) < 10:
        return None
...
def fetch_fallback_tickers():
    ...
    try:
        ...
        # S&P 500 (신뢰도 높은 GitHub Public CSV 데이터셋 활용)
...
def analyze():
    ...
    step1_liquidity, step2_price, step3_rsi_vol, final_pass = 0, 0, 0, 0
    msg_list = []
    
    # 분석 시작일: 3년 치 데이터 (2023-01-01)
    ...
    for ticker in tickers:
        try:
            ...
            if not (10 <= curr_price <= 300): continue
            step2_price += 1
            ...
            cond_rsi = rsi_val > 35
            if not (cond_vol and cond_rsi): continue
            step3_rsi_vol += 1
            ...
            if c1 and c2 and c3:
                ...
                if opt_mult is None:
                    stop_text = "<b>추천 불가</b> (과거 신호 10회 미만 부족)"
    ...
    footer = (f"\n<b>[진단 결과]</b>\n"
              f"* 0단계 (전체 유니버스): {total_scan}개\n"
              f"* 1단계 (유동성 필터): {step1_liquidity}개\n"
              f"* 2단계 (주당 가격 필터): {step2_price}개\n"
              f"* 3단계 (RSI/거래량 급감): {step3_rsi_vol}개\n"
              f"* 4단계 (최종 조건 부합): {final_pass}개")
