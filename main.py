...
SECTOR_CACHE_PATH = "sector_cache.json"  # [추가] 섹터 정보 캐시 파일
...
# =========================
def analyze():
    print(f"🚀 스캔 및 수신 시작 (KST): {datetime.now()}")
    ...
    # 상태 및 캐시 로드
    sector_cache = _load_json(SECTOR_CACHE_PATH, {}) # [추가] 섹터 캐시 로드
    ...
    # 모든 저장 완료
    _save_json(SECTOR_CACHE_PATH, sector_cache) # [추가] 섹터 캐시 저장
    save_pending_orders(pending)
    save_positions(positions)
